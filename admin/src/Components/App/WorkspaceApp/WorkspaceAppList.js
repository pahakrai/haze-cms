...
import WorkspaceCardItem from './WorkspaceCardItem';
import WorkspaceCardItemTitle from './WorkspaceCardItemTitle';
...
class WorkspacePhoneRegionList extends React.PureComponent {
  ...
  _renderItem = item => {
    ...
    return item === 'header' ? (
      <WorkspaceCardItemTitle key={item} intl={intl} />
    ) : (
      <WorkspaceCardItem
        key={item._id}
        intl={intl}
  ...
  render() {
    const { workspaceApps, gutter } = this.props;

    return (
      <List
        grid={{ gutter: gutter || 16, sm: 1, md: 1 }}
        dataSource={['header', ...workspaceApps]}
