...
export function* updateUserProfile(api, { data }) {
  ...
  const vehicle = dataVehicle ? { ...dataVehicle } : dataVehicle;
  // toast
  ...
  if (driverFiles && driverFiles[0]) {
    const { toastId, onUploadProgress } = formatUploadProgress(
      <FormattedMessage id="display_driver_licenseCardImage" />
    );
    const response = yield call(
      DriverService.updateDriverLicenseCardImage,
      driver._id,
      driverFiles,
      onUploadProgress
    );
    if (response && response.ok) {
      toast.update(toastId, {
        render: (
          <FormattedMessage id="display_driver_licenseCardImage_upload_success" />
        ),
      ...
      toast.update(toastId, {
        render: (
          <FormattedMessage id="display_driver_licenseCardImage_upload_failure" />
        ),
  ...
  // Update vehicle pictures
  if (vehicleFiles && vehicleFiles[0]) {
    const { toastId, onUploadProgress } = formatUploadProgress(
      <FormattedMessage id="display_vehicle_image" />
    );
    const response = yield call(
      VehicleService.updateVehicleImage,
      vehicle._id,
      vehicleFiles,
      onUploadProgress
    );
    const newImages = hasIn(response, 'data.images')
      ? response.data.images
      : [];

    if (vehicle) {
      vehicle.images = [...newImages, ...vehicle.images];
    }

    if (response && response.ok) {
      toast.update(toastId, {
        render: <FormattedMessage id="display_vehicle_image_upload_success" />,
        type: toast.TYPE.SUCCESS,
        autoClose: 1500
      });
    } else {
      toast.update(toastId, {
        render: <FormattedMessage id="display_vehicle_image_upload_failure" />,
        type: toast.TYPE.ERROR,
        autoClose: 1500
      });
    }
  ...
  if (vehicleRegDocImages && vehicleRegDocImages[0]) {
    const { toastId, onUploadProgress } = formatUploadProgress(
      <FormattedMessage id="display_vehicle_reg_doc_image" />
    );
    const response = yield call(
      VehicleService.updateVehicleRegistrationDoc,
      vehicle._id,
      vehicleRegDocImages,
      onUploadProgress
    );
    ...
    if (response && response.ok) {
      toast.update(toastId, {
        render: (
          <FormattedMessage id="display_vehicle_reg_doc_image_upload_success" />
        ),
        type: toast.TYPE.SUCCESS,
        autoClose: 1500
      });
      ...
      toast.update(toastId, {
        render: (
          <FormattedMessage id="display_vehicle_reg_doc_image_upload_failure" />
        ),
        type: toast.TYPE.ERROR,
        autoClose: 1500
      });
