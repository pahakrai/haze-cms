...
class MongodbHelper {
   ...
   */
  public _castPaginateOptions(query: any): object {
    const options: {
      offset?: number;
      page?: number;
      limit: number;
  ...
  public formatRangeQuery(
    ...
    if (startTime && endTime) {
      ...
      $or.push({[startField]: {$lte: startTime}, [endField]: {$gte: endTime}});
    } else if (startTime) {
      // only start time
      ...
      $or.push({[startField]: {$lte: endTime}, [endField]: {$lte: endTime}});
      //  doc startTime and endTime wraps query endTime
      $or.push({[startField]: {$lte: endTime}, [endField]: {$gte: endTime}});
    } else {
