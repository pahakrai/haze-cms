...
class PageFormContainer extends React.PureComponent {
  ...
  onSubmitSuccess() {
    const { history, onSubmitSuccess, isTemplate, isSection } = this.props;
    history.push(
      isTemplate ? '/page-templates' : isSection ? '/page-section' : '/pages'
    );
  ...
  getInitialValues = () => {
    const { page, updateMode, type, isTemplate } = this.props;
    if (updateMode && page) {
      const layout =
